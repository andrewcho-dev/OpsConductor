...
class TargetsServiceClient(BaseServiceClient):
    ...
    async def get_target(self, target_id: int, correlation_id: Optional[UUID] = None) -> Optional[Dict[str, Any]]:
        """Get target by ID"""
        try:
            response = await self.get(f"/api/v1/targets/{target_id}", correlation_id=correlation_id)
    ...
    async def list_targets(
        ...
        try:
            ...
            response = await self.get("/api/v1/targets", params=params, correlation_id=correlation_id)
    ...
    async def get_target_connection_methods(
        ...
        try:
            response = await self.get(f"/api/v1/targets/{target_id}/connection-methods", correlation_id=correlation_id)
    ...
    async def test_target_connection(
        ...
        try:
            ...
            response = await self.post(
                f"/api/v1/targets/{target_id}/test-connection",
                data=data,
                correlation_id=correlation_id
            )
    ...
    async def get_target_credentials(
        ...
        try:
            response = await self.get(
                f"/api/v1/targets/{target_id}/connection-methods/{connection_method_id}/credentials",
                correlation_id=correlation_id
            )
    ...
    async def get_targets_by_type(
        ...
        try:
            response = await self.get(f"/api/v1/targets?type={target_type}", correlation_id=correlation_id)
    ...
    async def get_healthy_targets(self, correlation_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
        ...
        try:
            response = await self.get("/api/v1/targets?status=healthy", correlation_id=correlation_id)
    ...
    async def get_target_health_status(
        ...
        try:
            response = await self.get(f"/api/v1/targets/{target_id}/health", correlation_id=correlation_id)
    ...
    async def get_targets_summary(self, correlation_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        try:
            response = await self.get("/api/v1/targets/summary", correlation_id=correlation_id)

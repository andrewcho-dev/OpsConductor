...
class AuthServiceClient:
    ...
    def __init__(self, auth_service_url: str = "http://auth-service:8000"):
...
def get_auth_client() -> AuthServiceClient:
    ...
    if _auth_client is None:
        # Use environment variable or fallback to correct default
        import os
        auth_url = os.getenv("AUTH_SERVICE_URL", "http://auth-service:8000")
        _auth_client = AuthServiceClient(auth_url)
    return _auth_client
...
async def get_current_user(
    ...
    try:
        ...
        # Get user information from validation result
        user_info = validation_result.get("user")
        logger.info(f"AUTH VALIDATION RESULT: {validation_result}")

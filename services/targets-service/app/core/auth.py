...
async def get_current_user(
    ...
    try:
        ...
        # Log the user data to debug the permission issue  
        logger.info(f"USER DATA RECEIVED: {current_user}")
        return current_user
...
def require_permission(permission: str):
    ...
    async def permission_checker(
        ...
        # Check if user has required permission or is admin/administrator
        user_role = current_user.get("role", "")
        is_admin = ("admin" in user_roles or 
                   user_role in ["admin", "administrator"] or
                   "administrator" in user_roles)
        
        # Debug logging (using INFO level to ensure it shows)
        logger.info(f"PERMISSION CHECK - User: {current_user.get('id')}, Role: {user_role}, Roles: {user_roles}, Permissions: {user_permissions}, Required: {permission}, Is Admin: {is_admin}")

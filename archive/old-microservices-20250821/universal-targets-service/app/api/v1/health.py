"""
Health check endpoints for Universal Targets Service
"""
...
async def detailed_health_check(db: Session = Depends(get_db)):
    ...
    # Check if we can query targets table
    try:
        result = db.execute(text("SELECT COUNT(*) FROM targets"))
        target_count = result.scalar()
        checks["targets_table"] = {
            "status": "healthy",
            "message": f"Targets table accessible, {target_count} targets found"
        }
    except Exception as e:
        checks["targets_table"] = {
            "status": "unhealthy",
            "message": f"Cannot access targets table: {str(e)}"
        }
        overall_status = "unhealthy"
...
async def readiness_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.execute(text("SELECT 1 FROM targets LIMIT 1"))
